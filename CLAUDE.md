...
| Folder | Contents |
|--------|----------|
| `USERLIST/` | **2,526** Artemis `.ncl` files — all patches merged into one folder |
| `archive/` | Old `USERLIST_OLD/`, `USERLIST_RISKY/`, `USERLIST_TESTED/` + research artifacts |

All patches from the former three USERLIST variants are now in a single `USERLIST/`. Patches confirmed on real PS3 hardware are marked `[Tested]` in the cheat name. Version-mismatched patches from the former risky mode are labeled `v01.XX (RPCS3)` inline.
...
`PATCHED_GAMES.md` (now in `archive/`) listed every `.ncl` file that received an RPCS3 patch — it was accurate for the pre-merge USERLIST and can be regenerated from `conversion_report.json` if needed.
